...
func getUserSessionCookie(r *http.Request, cluster string) (*http.Cookie, error) {
	ocpCookie, ocpCookieErr := r.Cookie(openshiftSessionCookieName + cluster)
...
func getOpenShiftUser(r *http.Request) (*User, error) {
	...
	cookie, err := getUserSessionCookie(r, cluster)
	if err != nil {
		return nil, err
	}
	if cookie.Value == "" {
		return nil, fmt.Errorf("unauthenticated, no value for cookie %s or %s", openshiftSessionCookieName + cluster, acmSessionCookieName)
	}
